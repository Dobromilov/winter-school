...
Само занятие на леции было достаточно понятным, а вот на практике не получалось нормально предстить работу, т.к. некоторые понятия были не так глубоко раскрыты
к примеру: нам рассказали что информацию мы будем должны класть последовательно добавляя в конец, но при этом, нам не говорили как это будет именно внутри выглядеть, из-за чего складывалось непонимание  реализацией структуры

сегдня была реализованна структура записи элемента в table.ю дальше понимание не пошло в реализации